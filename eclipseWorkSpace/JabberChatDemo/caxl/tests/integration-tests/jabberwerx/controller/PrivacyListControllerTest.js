/**
 * filename:        PrivacyListControllerTest.js
 *
...
jabberwerx.$(document).ready( function() {
    ...
    test("test block and presence", function() {
        ...
        var updateCB = function(err) {
            ...
            var actRecipients = prsCB.recipients || {length: 0};
            var expRecipients = arguments.callee.recipients || {length: 0};
    ...
    testCount++;
    test("test block stanzas", function() {
        //prep an entity
        var entities = [
            new jabberwerx.TemporaryEntity("bljid1@example.com"),
            new jabberwerx.TemporaryEntity("bljid2@example.com"),
            new jabberwerx.TemporaryEntity("bljid3@example.com"),
            new jabberwerx.TemporaryEntity("bljid4@example.com"),
            new jabberwerx.TemporaryEntity("bljid5@example.com")
        ];
        for (var idx = 0; idx < entities.length; idx++) {
            var ent = entities[idx];
            client.entitySet.register(ent);
        ...
        updateCB.recipients = {
            "jid:bljid1@example.com": true,
            "jid:bljid2@example.com": true,
            "jid:bljid3@example.com": true,
            "jid:bljid4@example.com": true,
            "jid:bljid5@example.com": true,
            length: 2
        };
        ...
        var fetchCB = function(list, err) {
            ...
            blist = list;
            list.blockJid("bljid1@example.com", [jabberwerx.PrivacyList.PRESENCE_IN]);
            list.blockJid("bljid2@example.com", [jabberwerx.PrivacyList.PRESENCE_OUT]);
            list.blockJid("bljid3@example.com", [jabberwerx.PrivacyList.MESSAGE]);
            list.blockJid("bljid4@example.com", [jabberwerx.PrivacyList.IQ]);
            list.blockJid("bljid5@example.com", [jabberwerx.PrivacyList.ALL]);
            list.update(updateCB);
        }
        privacy.fetch("JWA Privacy Blocking Stanzas", fetchCB);
    });
});
