...
jabberwerx.$(document).ready(function() {
    ...
    test ("Test valid connection", function() {
        ...
        equals(client.fullJid, null, "null fullJid when disconnected");
        var timing = function() {
            client.disconnect();
            start();
        };
        var arg = {
            successCallback: function() {
                ...
                ok(!client.connectedUser.jid.getResource(),
                   "connectedUser doesn't have a resource.");
                equals(client.connectedUser.jid.getDomain(),
                       client.connectedServer.jid.getDomain(),
                       "connectedUser and connectedServer have the same domain.");
                equals(client.fullJid,
                       client.connectedUser.jid.toString() + "/" + client.resourceName,
                       "fullJid is correct");
    ...
    test ("Test clientConnected and clientDisconnected events", function() {
        stop();

        var client = new jabberwerx.Client("client-integration-test");

        var startTestsAgain = function() {
            start();
        };

        client.event("clientConnected").bind(function(evt) {
            ok(true, "Client is connected");
            client.disconnect();
        });

        client.event("clientDisconnected").bind(function(evt) {
            ok(true, "Client is disconnected");
            setTimeout(startTestsAgain,3000);
        });

        client.connect("jwtest0@example.com", "test");

        // If all tests pass, 2 assertions are expected
        expect(2);
    });
    ...
    test ("Test in-band registration with credentials", function() {
        ...
        var testFullJid = testJid + "/" + client.resourceName;
        var arg = {

            successCallback: function() {
                testClientState(client, true);
                equals(client.fullJid, testFullJid, "Correct jid used to login");
                start();
                client.disconnect();
            },
    ...
    test ("Test in-band registration with no credentials - anonymous login", function() {
        ...
        var arg = {
            ...
            successCallback: function() {
                testClientState(client, true);
                ok( /^caxl_.*/.exec(client.fullJid), "jid starts with correct prefix");
                start();
                client.disconnect();
            },
    ...
    test("Test iq callback basics", function() {
        stop(5000);

        var client = new jabberwerx.Client("client-integration-test");
        var testJid = "example.com";
        var query = new jabberwerx.NodeBuilder("{http://jabber.org/protocol/disco#info}query").
                    data;
        var iqResult = null;
        var cb = function(iq) {
            iqResult = iq;
        ...
        var evtCB = function(evt) {
            if (!iqResult) {
                return;
            }
            if (iqResult === evt.data) {
                ok(false, "event '" + evt.name + "' still triggered after sendIq callback");
            }
        };
        client.event("iqReceived").bind(evtCB);
        client.event("afterIqReceived").bind(evtCB);

        var arg = {
            successCallback: function() {
                client.sendIq("get", testJid, query, cb);
            },
            errorCallback: function(err) {
                fail("client did not connect successfully");
                start();
            }
        };
        client.connect(testUserJID(1), defaults.password, arg);
    });
    ...
    test("Test sending stanzas before disconnect.", function() {
        stop();
        ...
        var timing = function() {
            ...
            ok(messagesReceived, "Messages Received");
            client1.disconnect();
            client2.disconnect();
            setTimeout(start, 1000);
        };
        ...
        arg1 = {
            successCallback: function() {
                client2.event("afterMessageReceived").bind(function(evt) {
                    numMessages--;
                ...
                numMessages++;
                client1.sendMessage(testUserJID(2), "before disconnect.");
                numMessages++;
                client1.sendMessage(testUserJID(2), "before disconnect 2.");
                client1.disconnect();
    ...
    test("Test request time from entity", function() {
    stop();
        var client1 = new jabberwerx.Client("time-request-client1");
        var client2 = new jabberwerx.Client("time-request-client2");

        var arg1 = {
                successCallback: function() {
                    ok(true, "client1 connected succesfully");
                    client2.connect(testUserJID(2), defaults.password, arg2);
                },
                errorCallback: function(evt) {
                    ok(false, "client1 did not connect ");
                    start();
                }
            };
        ...
        var arg2 = {
                successCallback: function() {
                    ok(true, "client2 connected succesfully");

                    var jid2 = client2.fullJid;
                    var query = new jabberwerx.NodeBuilder("{urn:xmpp:time}time");

                    client1.sendIq("get", jid2, query.data, function(stanza) {
                        var iq = new jabberwerx.IQ(stanza);
                        if (iq.isError()) {
                            ok(false, "error requesting time from entity");
                        } else {
                            ok(true, "time request from entity successful");
                        }
                    });
                ...
                errorCallback: function(evt) {
                    ok(false, "client2 did not connect ");
                    start();
                }
            };
