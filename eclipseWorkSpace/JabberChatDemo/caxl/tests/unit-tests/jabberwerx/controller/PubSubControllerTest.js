...
jabberwerx.$(document).ready(function() {
    ...
    test("Test node creation/access", function() {
        ...
        ok(caught, "fetchConfig expected error caught");

        try {
            caught = false;
            node.fetchConfig(function(frm, err){});
        } catch (ex) {
            caught = true;
            ok(ex instanceof jabberwerx.Client.NotConnectedError, "exception is NotConnectedError");
        }
        ok(caught, "fetchConfig expected error caught");
        ...
        ok(caught, "applyConfig expected error caught");

        try {
            caught = false;
            node.applyConfig(null, {});
        } catch (ex) {
            caught = true;
            ok(ex instanceof TypeError, "exception is TypeError");
        }
        ...
        try {
            caught = false;
            node.applyConfig(null, function(frm, err){});
        } catch (ex) {
            caught = true;
            ok(ex instanceof jabberwerx.Client.NotConnectedError, "exception is NotConnectedError");
        }
