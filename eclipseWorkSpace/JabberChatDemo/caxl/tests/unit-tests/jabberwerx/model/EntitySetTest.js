...
jabberwerx.$(document).ready( function() {
    ...
    test("Test Each", function() {
        ...
        ok(visited.length >= 2, "2 or more entities in set");
        var visitedCancel = [];
        entitySet.each(function(ent) {
            visitedCancel.push(ent);
            return false; // end iterating
        });
        equals(1, visitedCancel.length, "test only one visited after cancel");
        var contacts = [];
        // minimal distribution does not include Contact which changes the
        //number of qualifying entities. When jabberwerx.Contact exists it
        //each entity in the set is checked for instanceof Contact and none
        //passes. When jabberwerx.Contact is undefined (min distribution) each
        //entity is checked for instanceof jabberwerx.Entity and all pass
        entitySet.each(
                function(contact) {
                    contacts.push(contact);
                },
                jabberwerx.Contact);
        equals(jabberwerx.Contact === undefined ? 2 : 0, contacts.length, "test no contact in entity set");
    });
