...
jabberwerx.$(document).ready(function() {
    ...
    test("Test resource-/primaryPresenceChanged Event Firing", function() {
        ...
        //use the same Date for all priority 2 presences to test list insertion
        //last update wins ties
        var dt = new Date();
        ...
        //order is p6 > p5 > p2 > p3 > p1 > p4 based on priority & timestamp
        ...
        ok(resourceFn.nowAvailable, "nowAvailable is true");
        equals(primaryFn.triggered, false, "primaryPresenceChanged event NOT triggered");
        //p2 is primary, p6 becomes primary on unavail
        resourceFn.triggered = 0;
        primaryFn.triggered = false;
        p2 = p2.clone();
        ...
        equals(primaryFn.fullJid.toString(), p6.getFrom());
        ok(primaryFn.presence === p6, "primary presence is presence5 (xa)");
        //p5 == p6 but added before, primary remains p6 on unavail
        resourceFn.triggered = 0;
        primaryFn.triggered = false;
        p5 = p5.clone();
        ...
        ok(resourceFn.presence === p5, "resource presence is presence4 (away)");
        ok(!resourceFn.nowAvailable, "nowAvailable is false");
        equals(primaryFn.triggered, false, "primaryPresenceChanged event did not trigger");
        equals(primaryFn.fullJid.toString(), p6.getFrom());
        ok(primaryFn.presence === p6, "primary presence is still presence5 (xa)");
        //p6 is primary, p3 (priotity == 1) becomes primary on unavail
        resourceFn.triggered = 0;
        primaryFn.triggered = false;
        p6 = p6.clone();
