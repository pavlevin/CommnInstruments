...
jabberwerx.$(document).ready(function() {
    ...
    test("Test Create Using DOM", function() {
        ...
        same(form.getFORM_TYPE(), "jabber:bot", "XDataForm FORM_TYPE matches");
   ...
   test("Test Create Using no params", function() {
    ...
    test("Test setters and getters", function() {
        ...
        ok(!form.getFORM_TYPE(), "no FORM_TYPE");
        ...
        form.setFORM_TYPE("urn:bogus:formtype");
        same(form.getFORM_TYPE(), "urn:bogus:formtype", "FORM_TYPE matches");
    });
    test("Test FORM_TYPE getter and setter", function() {
        var form = new jabberwerx.XDataForm("form", null); 
        
        ok(!form.getFORM_TYPE(), "no FORM_TYPE");

        ok(!form.getFieldByVar("FORM_TYPE"), "no FORM_TYPE field");
        form.setFORM_TYPE("urn:some:formtype");
        same(form.getFORM_TYPE(), "urn:some:formtype", "FORM_TYPE matches");

        var fld = form.getFieldByVar("FORM_TYPE");
        ok(fld, "FORM_TYPE field exists");
        same(fld.getType(), "hidden", "FORM_TYPE is hidden");
        same(fld.getValues().length, 1, "FOMRM_TYPE field has one value");
        same(fld.getValues()[0], "urn:some:formtype", "FORM_TYPE field value matches");

        form.setFORM_TYPE(null);
        ok(!form.getFORM_TYPE(), "no FORM_TYPE");
        ok(!form.getFieldByVar("FORM_TYPE"), "no FORM_TYPE field");
    });
   ...
   test("Test Set Field Value", function() {
    ...
    test("Test Set Field Value (non-existent)", function() {
        var formXml ="<x xmlns='jabber:x:data' type='form'></x>";
        var formElem = jabberwerx.util.unserializeXML(formXml);
        var form = new jabberwerx.XDataForm(null, formElem);

        form.setFieldValue("invitelist","1@test.com");
        var field = form.getFieldByVar("invitelist");
        ok(form.getFieldValue("invitelist") == "1@test.com", "field value matches");        
        ok(field, "field exists");
        ok(field.getType(), "text-single", "field type is defaulted to 'text-single'");
        
        form = new jabberwerx.XDataForm(null, formElem);
        form.setFieldValue("invitelist","1@test.com","jid-multi");
        var field = form.getFieldByVar("invitelist");
        ok(form.getFieldValue("invitelist") == "1@test.com", "field value matches");        
        ok(field, "field exists");
        ok(field.getType(), "jid-multi", "field type is defaulted to 'jid-multi'");
    });
